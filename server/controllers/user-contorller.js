...
const getUserById = async(req, res) => {
	try{
		...
		res.status(400).json({result:"failed", message:"Cannot find user with that id"})
	}
}

const authLogin = async (req, res) => {
	const foundEmail = await User.findOne({email: req.body.email})
	if (!foundEmail) return res.status(401).json({message:'Invalid email or password.'})

	const pwValid = await bcrypt.compare(req.body.password, foundEmail.password)
	if(!pwValid) return res.status(401).json({message: 'Invalid email or password.'})

	const {pw, ...modifiedUser} = foundEmail
	const token = jwt.sign({_id: foundEmail._id, email: foundEmail.email}, process.env.JWT_SECRET)
